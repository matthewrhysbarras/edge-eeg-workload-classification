...
static const uint32_t MAGIC = 0x31574245UL;
...
void setup() {
  ...
  Serial.println("# esp32_window_replay_full ready");
  Serial.println("# demo_type=real-time embedded inference on replayed/pre-recorded preprocessed EEG windows");
  Serial.println("# columns=replay_row_index,model_index,pred_label,max_abs_feature_error,max_feature_error_index,mean_abs_feature_error,rmse_feature_error,logit_0,logit_1,logit_2,prob_0,prob_1,prob_2,feature_us,classifier_us,total_us");
}

void loop() {
  uint32_t magic = 0;
  if (!read_magic_resync(magic)) return;
  uint32_t replay_row_index = 0;
  uint16_t model_index = 0;
  uint16_t true_label_index = 0;
  double sfreq_hz = 250.0;
  if (!read_exact(reinterpret_cast<uint8_t*>(&replay_row_index), sizeof(replay_row_index))) return;
  if (!read_exact(reinterpret_cast<uint8_t*>(&model_index), sizeof(model_index))) return;
  if (!read_exact(reinterpret_cast<uint8_t*>(&true_label_index), sizeof(true_label_index))) return;
  ...
  if (!read_exact(reinterpret_cast<uint8_t*>(ref_features), sizeof(ref_features))) return;
  if (!read_exact(reinterpret_cast<uint8_t*>(eeg), sizeof(eeg))) return;
  ...
  for (int i = 0; i < N_FEATURES; i++) {
    const float err = fabsf(features[i] - ref_features[i]);
    if (isfinite(err)) {
      if (err > max_abs_err) {
        max_abs_err = err;
        max_err_index = i;
  ...
  const int pred = classify(model_index, features, logits, probs);
  ...
  Serial.print(',');
  Serial.print(max_abs_err, 9);
  Serial.print(',');
  Serial.print(max_err_index);
  Serial.print(',');
  Serial.print(sum_abs_err / float(N_FEATURES), 9);
  Serial.print(',');
  Serial.print(sqrtf(sum_sq_err / float(N_FEATURES)), 9);
  for (int c = 0; c < N_CLASSES; c++) {
    Serial.print(',');
